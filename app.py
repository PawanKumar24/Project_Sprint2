...
app = Flask(__name__)
app.secret_key = 'hellopawankotak'

# Establish a connection to the database
mydb = pymysql.connect(
  host = "127.0.0.1",
  user = "root",
  password = "Foodlover.55",
  database = "mytestdb"
)
...
@app.route('/cargo/update', methods=['GET'])
def update_cargo_form():
    return render_template('update_cargo.html')
#update cargo items
...
def add_captain():
    ...
    try:
        ...
        designation = request.form.get('rank')
        homeplanet = request.form.get('homeplanet')
        
        # Inserting the data into the 'captain' table
        query = f"INSERT INTO captain (firstname, lastname, designation, homeplanet) VALUES ('{firstname}', '{lastname}', '{designation}', '{homeplanet}')"
...
def update_captain():
    try:
        ...
        designation = request.form['rank']
        home_planet = request.form['homeplanet']
        
        
        # Update the captain details
        update_query = f"UPDATE captain SET firstname = '{first_name}',lastname = '{last_name}',designation = '{designation}',homeplanet = '{home_planet}'"
        update_query += f" WHERE id = {id}"
        cursor.execute(update_query)
        mydb.commit()
        cursor.close()
...
def spaceship():
    try:
        spaceship_data = get_spaceShip()
        return render_template('spaceship.html', spaceship_data = spaceship_data)
    except Exception as e:
        # Return an error message if there's any exception
        print("Error occurred: ", e)
        return jsonify({'error ': str(e)})
    ...
    try:
        with mydb.cursor() as cursor:
            ...
            for item in rows:
                ...
                spaceship_data.append(spaceship_item)
            print(spaceship_data)
...
@app.route('/spaceship/update', methods=['PUT'])
def update_spaceship():
    try:
        id = request.form('id')
        max_weight = request.form('maxweight')
        captain_id = request.form('captainid')
        # Get the connection cursor
        cursor = mydb.cursor()

        # Update the spaceship details
        update_query = f"UPDATE spaceship SET maxweight = '{max_weight}',captainid = '{captain_id}'"
        update_query += f" WHERE id = {id}"
